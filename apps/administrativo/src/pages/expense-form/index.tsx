...
import { ChevronLeftIcon, ReceiptIcon, SaveIcon } from 'lucide-react'
...
export const ExpenseForm = () => {
  ...
  async function addOrUpdateExpense(values: ExpenseData) {
    try {
      ...
      await api[params.id ? 'put' : 'post'](params.id ? 'expense.update' : 'expense.add', formData, {
        headers: { Authorization: `Bearer ${token}` },
      })

      toast.success(`Despesa ${params.id ? 'atualizada' : 'registrada'} com sucesso!`)
      pushTo(-1)
    } catch (error) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      params.id ? api.get(`expense.key/${params.id}`, config) : Promise.resolve({ data: null }),
    ])
  ...
  return (
    <>
      <Helmet>
        <title>Despesa - APA Control</title>
      </Helmet>
      <Card>
        <CardHeader>
          <CardTitle>
        ...
        <FormProvider {...expenseForm}>
          <form autoComplete="off" onSubmit={handleSubmit(addOrUpdateExpense)}>
            <CardContent>
