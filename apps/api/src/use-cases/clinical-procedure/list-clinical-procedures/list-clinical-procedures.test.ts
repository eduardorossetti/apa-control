...
describe('List clinical procedures', () => {
  ...
  beforeAll(async () => {
    ...
    await db.insert(clinicalProcedure).values({
      animalId: animal.id,
      procedureTypeId: procedureType.id,
      appointmentId: createdAppointment.id,
      employeeId,
      procedureDate: new Date(),
      description: 'Procedimento para listagem',
  ...
  it('should list clinical procedures', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/clinical-procedure.list?page=1&perPage=10&procedureDateStart=2020-01-01&procedureDateEnd=2100-12-31',
  ...
  it('should filter by animalName returning empty', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/clinical-procedure.list?page=1&perPage=10&procedureDateStart=2020-01-01&procedureDateEnd=2100-12-31&animalName=xyz-nonexistent',
      headers: { authorization: `Bearer ${token}` },
    })
  ...
  it('should return 422 when procedureDateStart is after procedureDateEnd', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/clinical-procedure.list?page=1&perPage=10&procedureDateStart=2100-01-01&procedureDateEnd=2020-12-31',
      headers: { authorization: `Bearer ${token}` },
    })
