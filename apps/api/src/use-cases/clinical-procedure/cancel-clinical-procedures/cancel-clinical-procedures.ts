...
export class CancelClinicalProceduresUseCase {
  ...
  async execute(data: CancelClinicalProceduresData, employeeId: number): Promise<void> {
    ...
    if (procedures.some((procedure) => procedure.status !== ProcedureStatus.SCHEDULED)) {
      throw new ApiError('Apenas procedimentos clínicos agendados podem ser cancelados.', 409)
    }
      ...
      for (const procedure of procedures) {
        await this.animalHistoryRepository.create(
          new AnimalHistory({
            ...
            action: 'clinical-procedure.cancelled',
            description: `Procedimento ${procedureTypeById.get(procedure.procedureTypeId)} cancelado`,
            oldValue: null,
            newValue: null,
            createdAt: new Date(),
          }),
