...
describe('Cancel adoption', () => {
  ...
  it('should cancel adoption successfully', async () => {
    ...
    const response = await app.inject({
      ...
      url: '/adoption.cancel',
      headers: { authorization: `Bearer ${token}` },
      payload: { id, reason: 'Motivo do cancelamento' },
    })

    expect(response.statusCode).toBe(204)
  })

  it('should return 400 when adoption is already completed', async () => {
    const id = await createAdoption(AdoptionStatus.COMPLETED)

    const response = await app.inject({
      method: 'PUT',
      url: '/adoption.cancel',
      headers: { authorization: `Bearer ${token}` },
      payload: { id, reason: 'Tentar cancelar concluída' },
    })
  ...
  it('should return 404 when adoption does not exist', async () => {
    const response = await app.inject({
      ...
      url: '/adoption.cancel',
      headers: { authorization: `Bearer ${token}` },
      payload: { id: 99999, reason: 'Não existe' },
    })
