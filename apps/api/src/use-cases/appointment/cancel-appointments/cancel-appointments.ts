...
import type { CancelAppointmentsData } from './cancel-appointments.dto'

export class CancelAppointmentsUseCase {
  constructor(
  ...
  async execute(data: CancelAppointmentsData, employeeId: number): Promise<void> {
    ...
    if (appointments.some((appointment) => appointment.status !== AppointmentStatus.SCHEDULED)) {
      throw new ApiError('Apenas consultas agendadas podem ser canceladas.', 409)
    }
    ...
    await db.transaction(async (tx) => {
      await this.appointmentRepository.updateStatusByIds(data.ids, AppointmentStatus.CANCELLED, tx)
      await this.reminderRepository.deleteByEntity(ReminderEntityType.APPOINTMENT, data.ids, tx)
      ...
      for (const appointment of appointments) {
        await this.animalHistoryRepository.create(
          new AnimalHistory({
            ...
            description: `Consulta ${appointmentTypeById.get(appointment.appointmentTypeId)} cancelada`,
            oldValue: null,
            newValue: null,
            createdAt: new Date(),
          }),
